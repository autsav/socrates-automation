...
Usage:
    from src.visual.brand_design import BrandDesign, MOOD_PALETTES
...
_TEXTURE_DIR = Path(__file__).parent.parent.parent / "textures"
...
# ── Point 9: Dynamic Layout Variations ───────────────────────────────────────
# 5 layout templates for quote panel placement.
# Each defines panel top/bottom as fraction of image height.
...
LAYOUT_TEMPLATES = {
    "centered": {
        ...
        "line_y_top": 0.28,
        "line_y_bottom": 0.72,
        "description": "Default — balanced centered composition",
    },
    "top_weighted": {
        "panel_top": 0.10,
        "panel_bottom": 0.60,
        ...
        "line_y_top": 0.13,
        "line_y_bottom": 0.57,
        "description": "Panel in top 60% — works well with bottom-heavy backgrounds",
    },
    "bottom_weighted": {
        "panel_top": 0.42,
        "panel_bottom": 0.90,
        ...
        "line_y_top": 0.45,
        "line_y_bottom": 0.87,
        "description": "Panel in bottom half — foreground subject in upper frame",
    },
    "full_bleed": {
        "panel_top": 0.05,
        "panel_bottom": 0.95,
        ...
        "line_y_top": 0.08,
        "line_y_bottom": 0.92,
        "description": "Near full-height panel — immersive, dark editorial feel",
    },
    "split": {
        "panel_top": 0.30,
        "panel_bottom": 0.70,
        ...
        "line_y_top": 0.32,
        "line_y_bottom": 0.68,
        "description": "Narrow band — lets background breathe above and below",
    },
}

LAYOUT_ORDER = list(LAYOUT_TEMPLATES.keys())  # rotation order


# ── Point 14: Seasonal/Contextual Design ─────────────────────────────────────

def get_season(month: int | None = None) -> str:
    """Return current season string: winter | spring | summer | autumn."""
...
SEASONAL_PALETTE_OVERRIDES = {
    "winter": {
        "primary": (8, 10, 20),
        "accent": (140, 180, 220),
        "description": "Cool blues — winter introspection",
    },
    "spring": {
        "primary": (10, 20, 10),
        "accent": (120, 200, 120),
        "description": "Soft greens — renewal and growth",
    },
    "summer": {
        "primary": (25, 15, 5),
        "accent": (255, 180, 60),
        "description": "Warm golds — energy and clarity",
    },
    "autumn": {
        "primary": (20, 12, 5),
        "accent": (200, 100, 40),
        "description": "Burnt oranges — depth and reflection",
    },
}


def get_seasonal_palette(mood: str, month: int | None = None) -> dict:
    """
    Point 14: Return palette merged with seasonal overrides.
    Winter → cooler blues. Summer → warmer golds.
    """
    base = dict(MOOD_PALETTES.get(mood, MOOD_PALETTES["dark_philosophical"]))
    season = get_season(month)
    overrides = SEASONAL_PALETTE_OVERRIDES.get(season, {})
    # Blend: only apply seasonal accent if mood is neutral-enough
    neutral_moods = {"calm_stoic", "stark_minimal"}
    if mood in neutral_moods:
        base.update(overrides)
    return base


def pick_layout(row_number: int = 0, mood: str = "") -> dict:
    """
    Point 9: Return a layout template, rotating by row_number for variety.
    Certain moods have preferred layouts:
      dark_philosophical → centered / split
      epic_warrior       → full_bleed
      cinematic_hopeful  → top_weighted
    """
    mood_preference = {
        "dark_philosophical": ["centered", "split"],
        "dramatic_ancient": ["centered", "bottom_weighted"],
        "cinematic_hopeful": ["top_weighted", "centered"],
        "epic_warrior": ["full_bleed", "centered"],
        "mystical_greek": ["split", "centered"],
        "calm_stoic": ["centered", "top_weighted"],
        "stark_minimal": ["split", "centered"],
    }
    candidates = mood_preference.get(mood, LAYOUT_ORDER)
    key = candidates[row_number % len(candidates)]
    return {**LAYOUT_TEMPLATES[key], "name": key}
