...
def _recent_trigger_posts(lookback_posts: int, db_path=DB_PATH) -> list[dict]:
    ...
    try:
        ...
        logger.info(f"[funnel] DB read failed ({e}) — empty sweep")
        return []
...
def _append_tally(tally: dict) -> None:
    try:
        ...
        logger.info(f"[funnel] tally write failed ({e})")
...
def run_funnel_sweep(cfg, *, lookback_posts: int = 10, db_path=DB_PATH,
    ...
    if not token:
        logger.warning("[funnel] META_ACCESS_TOKEN missing — nothing to do")
        _append_tally(tally)
        return tally
    ...
    for post in _recent_trigger_posts(lookback_posts, db_path):
        ...
        try:
            ...
            logger.info(f"[funnel] fetch failed for {post['post_id']} ({e}) — skipping post")
            continue
        for c in comments:
            ...
            try:
                ...
                logger.info(f"[funnel] reply failed for comment {cid} ({e}) — continuing")
    _append_tally(tally)
    logger.info(f"[funnel] sweep: {tally}")
    return tally
