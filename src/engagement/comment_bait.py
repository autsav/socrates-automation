...
# ── Point 36: Reply Guy Strategy ─────────────────────────────────────────────

class ReplyGuyEngine:
    """
    Generate escalating reply templates for the first 15 minutes after posting.
    Psychology: early comment velocity signals to the algorithm that content is engaging.
    Strategy: reply to every comment with a more extreme, polarising take to
    generate reply-to-reply chains (reply threads boost reach significantly).
    """

    # Escalating reply templates by comment sentiment
    REPLIES = {
        "agreement": [
            "You already know it. The real question is — what are you going to do about it?",
            "Exactly. And most people would rather agree and scroll on. You won't.",
            "Say it louder. Most people need to hear this 10 times before it lands.",
            "This is the part where you either act or tell yourself you'll 'start Monday.'",
        ],
        "disagreement": [
            "I hear you. Challenge: name one thing you'd do differently if you believed this was true.",
            "Fair. What would you replace it with? Genuinely curious.",
            "That's a valid pushback. Here's why I still think {philosopher} got it right: discomfort.",
            "Disagree all you want. The question is still there when you wake up tomorrow.",
        ],
        "question": [
            "Great question. Short answer: the examined life is uncomfortable. That's the point.",
            "Here's a better question: what's stopping you from knowing the answer already?",
            "You're asking because part of you already knows. Trust that part.",
        ],
        "generic": [
            "This is the comment the algorithm needed. Thank you.",
            "You stopped scrolling. That's already different from 90% of people.",
            "Drop a 🔥 if this thread is hitting.",
            "The people who comment on posts like this are the ones who actually do things.",
        ],
    }

    def generate_reply(
        self,
        comment_type: str = "generic",
        philosopher: str = "Socrates",
        seed: int = 0,
    ) -> str:
        """
        Generate a reply for a given comment type.
        comment_type: "agreement" | "disagreement" | "question" | "generic"
        """
        if seed:
            random.seed(seed)
        pool = self.REPLIES.get(comment_type, self.REPLIES["generic"])
        reply = random.choice(pool)
        return reply.replace("{philosopher}", philosopher)
    ...
    def generate_reply_set(
        ...
        """
        Generate a full set of replies covering each comment type.
        Returns n replies for posting in first 15 minutes.
        """
        types = list(self.REPLIES.keys())
        replies = []
        for i in range(n):
            ctype = types[i % len(types)]
            replies.append(self.generate_reply(ctype, philosopher, seed=i))
        return replies


def get_reply_set(philosopher: str = "Socrates", n: int = 4) -> list[str]:
    """One-shot reply set for the reply guy strategy."""
    return ReplyGuyEngine().generate_reply_set(philosopher=philosopher, n=n)


# ── Point 42: Hook Performance Tracking ─────────────────────────────────────

def log_hook_performance(
    hook_text: str,
    hook_type: str,
    post_id: str,
    three_second_hold: float = 0.0,
) -> None:
    """
    Point 42: Log hook template performance for A/B testing.
    three_second_hold: fraction of viewers who watched past 3 seconds (0-1).
    Stored in analytics/hook_tracker if available.
    """
    try:
        from src.analytics.hook_tracker import record_hook_outcome
        # Convert 3s hold rate to a proxy reach score for storage
        proxy_reach = int(three_second_hold * 1000)
        record_hook_outcome(
            hook_id=hook_type,
            actual_saved=0,
            actual_comments=0,
            actual_reach=proxy_reach,
        )
    except Exception:
        pass  # Non-fatal; analytics tracking should never break the pipeline
