...
def test_any_reel_routes_to_pov():
    ...
    assert pipeline._reels_use_renderer(reel=True, carousel=False, renderer="ffmpeg")


def test_explicit_flags_still_force_pov():
    assert pipeline._reels_use_renderer(reel=False, carousel=False, renderer="hyperframes")
...
def test_image_and_carousel_are_not_forced_to_reel():
    ...
    assert not pipeline._reels_use_renderer(reel=True, carousel=True, renderer="hyperframes")